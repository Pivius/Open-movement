...
function SetSettings(ply, accel, airaccel, gravity, friction, canrj)
	if ply:accel() != accel then
		ply:accel(accel)
	end
	if ply:air_accel() != airaccel then
		ply:air_accel(airaccel)
	end
	if ply:gravity() != gravity then
		ply:gravity(gravity)
	end
	if ply:friction() != friction then
		ply:friction(friction)
	end
	if ply:can_rocketjump() != canrj then
		ply:can_rocketjump(canrj)
	end
end
...
local function Movement_Airmove(ply, mv, cmd)
	if ply:movement_mode() == util.Table.GetByValue(MOVEMENT_MODES, "Airmove") then
		SetSettings(ply, 10, 10, 600, 6, false)
		SetWalkSpeed(ply, 250)
		SetJumpVelocity(ply, 270)
		AutoHop(ply, mv, true)
...
hook.Add("SetupMove", "Core_Movement", function(ply, mv, cmd)
	...
	if ply:GetGroundEntity() != NULL then
		...
		-- 30 gain is normal.
		if ply:movement_mode() != util.Table.GetByValue(MOVEMENT_MODES, "CPMA") && ply:movement_mode() != util.Table.GetByValue(MOVEMENT_MODES, "VQ3") && ply:movement_mode() != util.Table.GetByValue(MOVEMENT_MODES, "Momentum") then
