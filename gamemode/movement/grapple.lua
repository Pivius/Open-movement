...
function PLAYER:Retract(mv, distToHook, tension, dirToHook)
  ...
  distToHook = self:RopeLength()  //Its the same
...
function Hook(ply, mv, cmd)
  ...
  */
  ...
  if ply:Grappling() || (CLIENT && (ply.plyTick == cmd:TickCount())) then
    //Trace
    local Intersect = {}
    Intersect.start = ply:EyePos()-Vector(0,0,7) //So it doesnt look really weird with jumping rope
  ...
  for k, intersect in pairs(ply:Intersections()) do
    if k >= 2 then
      ...
      Intersect.endpos = ply:EyePos()-Vector(0,0,7) //So it doesnt look really weird with jumping rope
      Intersect.filter = function(e1, e2)
        return !e1:IsPlayer()
      end
      local trace = util.TraceLine(Intersect)
